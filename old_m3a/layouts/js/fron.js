...
//answer 
    ...
    $('.fafafa #com-no').one("click", function () {
     ...
     $('.alls .rightanswer').one("click", function () {
...
$('#checked_2, .click').click(function () {
  $('html, body').animate({
    scrollTop: $('.login-page').offset().top
    }, 1000);
});
...
$('.confirm').click(function () {
		return confirm('لقد قمت بانشاء حساب انتظر التفعيل');
  });
  $('.confirm_2').click(function () {
		return confirm('يرجي قرائه كيفيه التسجيل في معلومات التسجيل ثم انشاء حساب وانتظار الموافقة');
	});  
//comment toggle
...
//bluecolor
$('.answerview').one("click", function () {
  $('this').prev().addClass('answerview');
});
//reg.php
$('.our_work h3').on("click", function () {
  $(this).next('div').toggle("slow");
});
