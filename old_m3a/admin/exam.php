			...
			<div class="container add text-right">
			 <h1 class="text-center">اضافة امتحان جديد</h1>                
				<form class="form-horizontal" action="?do=Insert" method="POST">
					...
					<div class="form-group form-group-lg">
						<label class="control-label">لنك الامتحان</label>
						<textarea
								class="form-control text-right" 
								name="link" 
								placeholder="اضف لنك الامتحان" ></textarea>
					</div>
					<!-- End Members Field -->
                    <!-- Start lesson Field -->
					<div class="form-group form-group-lg">
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (empty($formErrors)) {
					$stmt = $con->prepare("INSERT INTO 
						exams(exam_name, exam_date, categ_id, link, member_id, lesson_id)
						VALUES(:zname, now(), :zcat, :zlink, :zmember, :zlesson)");
					$stmt->execute(array(
						'zname' 	=> $name,
						'zcat'		=> $cat,
						'zlink'		=> $link,
