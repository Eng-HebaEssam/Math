				...
				<div class="container text-right">
					<form class="form-horizontal" action="?do=Update" method="POST">
						...
						<div class="form-group form-group-lg">
							...
							<!-- End Full Name Field -->
							<div class="form-group form-group-lg">
                                    <label class="control-label"> اختر الصف الدراسى </label>  
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (empty($formErrors)) {
					...
					if ($count == 1) {
						...
						$stmt = $con->prepare("UPDATE members SET username = ?, fullname = ?, password = ?, groupid = ? WHERE userid = ?");
						$stmt->execute(array($user, $name, $pass, $class, $id ));
						// Echo Success Message
